...
$rs=mysqli_query($conn,"select * from user_info where username='$username'");
if (mysqli_num_rows($rs)>0)
{
	echo "<br><br><br><div class=head1>Account already exists.</div>";
	echo "<br><div class=head1><a href=login.php>Login</a></div>";
	exit;
}
$query="insert into user_info(name,username,password,role) values('$name','$username','$password','$role')";
$rs=mysqli_query($conn,$query)or die("Could Not Perform the Query");
echo "<br><br><br><div class=head1> Hello $username, you successfully created account!</div>";
echo "<br><div class=head1>Please login using your username and password.</div>";
echo "<br><div class=head1><a href=login.php>Login</a></div>";
